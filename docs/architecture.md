...
- **robots.txt** at `/robots.txt` (`robots_txt` view in `config/urls.py`): disallows only `/admin/`; advertises the sitemap. `/users/` and `/api/` are deliberately **crawlable** — see noindex below. (Cloudflare prepends its own managed/content-signal block ahead of ours on the live response.)
