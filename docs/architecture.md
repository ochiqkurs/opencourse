...
- **Slow-network resilience:** a click on play before the IFrame API has bootstrapped is queued (`pendingPlay`) and flushed in `onReady` instead of being dropped, with the spinner as immediate feedback. The play/pause icon is **not** trusted to YT's state machinery, because on slow networks both the events *and* the `getPlayerState()` cache lag seconds behind reality or drop updates entirely (observed in prod: video visibly playing while the cache still says paused). A 250 ms ticker runs permanently from `onReady` and reconciles with two stronger truths: for `INTENT_GRACE_MS` (3 s) after an explicit play/pause the **user's intent** paints the icon outright, and beyond that a **moving playback clock** (`getCurrentTime()` delta between ticks, 0.05–1.5 s so seeks don't count) means "playing" regardless of what the cache claims. `BUFFERING` paints as playing when there is play intent; the spinner shows on `BUFFERING` only while the clock isn't moving.
