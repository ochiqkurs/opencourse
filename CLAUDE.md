...
- **SEO meta is centralized**: `base.html` `<head>` includes `templates/includes/seo_meta.html`, driven by the `learning.context_processors.seo` defaults. To customize a page's description / OG image / canonical, set `meta_description` / `og_title` / `og_image` / `og_type` in the **view context** (don't hand-write meta tags in templates). For structured data, build a dict in the view and render it with `{% jsonld data %}` inside `{% block extra_head %}`. Sitemap (`/sitemap.xml`), `robots.txt`, JSON-LD, and analytics are detailed in `docs/architecture.md` → **SEO / Discoverability**. New models that get their own page should add `get_absolute_url()` and, if public, a sitemap entry.
