...
### Tests
A small, focused Django `TestCase` suite lives in `learning/tests.py` — it locks in the progress/enrollment integrity guarantees (no progress on draft/archived courses, GET stays side-effect free, play enrolls+completes) and multi-select quiz grading (exact-set match). Run with `python manage.py test`. Coverage is intentionally narrow; most features are still verified manually via the browser. Add new tests here (or per-app `tests.py`) when changing progress, enrollment, certificate, or quiz logic.
...
- Rate limiting on `/api/auth/check/<token>/` (60 req/min), the code login (10/min per IP, `prefix='code'`), and the password login (10/min per IP, `prefix='login'`). Backed by a **DatabaseCache** (`CACHES` → `cache_table`) so limits are shared across Gunicorn workers and survive restarts — not the per-process default `LocMemCache`. `_client_ip` prefers `CF-Connecting-IP`, then the last `X-Forwarded-For` entry, then `REMOTE_ADDR`, to resist spoofing. Telegram avatars are downloaded server-side to local `/media/` (`_localize_avatar`) so the bot-token-bearing `api.telegram.org/file/bot<TOKEN>/…` URL is never stored or rendered to the browser.
